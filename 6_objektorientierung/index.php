...
try {
    $eintrag1 = new GradeEntry('Marcel Schranz', 'POS', 'm.s@gmx.at', 1, '29-11-2022');
    $eintrag1->setStudiname('Marcel');
    echo $eintrag1->getStudiname();
    $eintrag1->saveToSession();
    $eintrag1->saveToFile('testfile.txt');
    GradeEntry::getAllEntries();
} catch (Exception $ex) {
    echo $ex->getMessage();
    //TODO: Sinnvolle Fehlerbehandlung: Mail an Admin, qualifzierte Fehlermeldung auf Seite
}
        ...
        <form id="form_grade" action="index.php" method="post">
            ...
            <div class="row">
                ...
                <div class="col-sm-6 form-group">
                    ...
                    <input type="text" name="name" class="form-control <?= isset($errors['name']) ? 'is-invalid' : '' ?>" value="<?= htmlspecialchars($name) ?>" maxlength="20" required="required" />
            ...
            <div class="row">
                ...
                <div class="col-sm-4 form-group">
                    ...
                    <select name="subject" class="custom-select <?= isset($errors['subject']) ? 'is-invalid' : '' ?>" required="required">
                ...
                <div class="col-sm-4 form-group">
                    ...
                    <input type="number" name="grade" class="form-control <?= isset($errors['grade']) ? 'is-invalid' : '' ?>" value="<?= htmlspecialchars($grade) ?>" min="1" max="5" required="required" />
                </div>


                <div class="col-sm-4 form-group">
                    <label for="examDate">Prüfungsdatum*</label>
                    <input type="date" name="examDate" class="form-control <?= isset($errors['examDate']) ? 'is-invalid' : '' ?>" value="<?= htmlspecialchars($examDate) ?>" onchange="validateExamDate(this)" required="required" />
                </div>
